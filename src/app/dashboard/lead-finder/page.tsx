...
export default function LeadFinderPage() {
  ...
  // Compute scored map whenever leads / country / niche change
  useEffect(() => {
  ...
  const handleSearch = async (e?: React.FormEvent, searchNiche?: string, searchCity?: string, searchCountry?: string) => {
    ...
    try {
      ...
      if (response.ok && data.success && data.businesses?.length > 0) {
        ...
        localStorage.setItem('localradar_latest_country', finalCountry);
  ...
  const getDealValue = (bizId: string) =>
    scoredMap[bizId]?.dealValue?.formatted ??
    formatCurrency(opportunities[bizId]?.estimated_deal_value || 0, country);
  ...
  const formatMoney = (value: number) => formatCompactCurrency(value, country);
        ...
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
          {[
            { 
              ...
              val: formatMoney(stats.pipeline), 
            ...
            { 
              ...
              val: formatMoney(stats.weightedPipeline), 
